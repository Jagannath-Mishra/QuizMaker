...
public class QuizAttemptController {
	...
	public String doTest(@RequestParam HashMap<String, String> listOfParams, RedirectAttributes redirectAttributes) { 
		...
		Question questionFirst = questionRepository.getById(Long.parseLong((String)listOfParams.keySet().stream().findFirst().get()));
		...
		float totalScore = 0.0f ;
		 for (Map.Entry mapElement : listOfParams.entrySet()) {
	            ...
	            QuizAttempt quizAttempt = new QuizAttempt(loggedInUser, tests, question, optionBean, true, optionBean.isCorrect());
